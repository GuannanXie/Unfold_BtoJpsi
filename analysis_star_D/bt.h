...
Int_t bt::Cut(Long64_t entry)
{
   ...
   for(int i=0;i<np;++i)
   {
      ...
      if(fabs(pdgid[i])==11&&status[i]>0&&fabs(eta[i])<0.5&&ptTmp>3.&&ptTmp<10.)
      {
   ...
   for(int i=0;i<np;++i)
   {
      ...
      if(fabs(pdgid[i])==11&&status[i]>0&&fabs(eta[i])<0.5&&ptTmp>3.&&ptTmp<10.)//e
      {
         ...
         if(parentpt<14.5){
            decay_Dtoe_selected_rebin->Fill(parentpt,ptTmp+10.);
         }else{
            decay_Dtoe_selected_rebin->Fill(14.75,ptTmp+10.);
         }
      }
   }   
   return 1;
